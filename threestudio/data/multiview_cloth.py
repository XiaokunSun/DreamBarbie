...
class RD_MultiviewIterableDataset(IterableDataset):
    def __init__(self, cfg: Any) -> None:
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            rgb_frame_path = os.path.join(self.cfg.dataroot, frame["rgb_file_path"])
            img = cv2.imread(rgb_frame_path)[:, :, ::-1].copy()
            img = cv2.resize(img, (self.frame_w, self.frame_h))
            img: Float[Tensor, "H W 3"] = torch.FloatTensor(img) / 255
            rgb_frames_img.append(img)

            albedo_file_path = os.path.join(self.cfg.dataroot, frame["albedo_file_path"])
            img = cv2.imread(albedo_file_path)[:, :, ::-1].copy()
            img = cv2.resize(img, (self.frame_w, self.frame_h))
            img: Float[Tensor, "H W 3"] = torch.FloatTensor(img) / 255
            albedo_frames_img.append(img)
        ...
        self.rgb_frames_img: Float[Tensor, "B H W 3"] = torch.stack(rgb_frames_img, dim=0)
        self.albedo_frames_img: Float[Tensor, "B H W 3"] = torch.stack(albedo_frames_img, dim=0)
    ...
    def collate(self, batch):
        ...
        return {
            ...
            "mvp_mtx": self.mvp_mtx[start_idx : end_idx],
            "c2w": self.frames_c2w[start_idx : end_idx],
            "camera_positions": self.frames_position[start_idx : end_idx],
            "light_positions": self.light_positions[start_idx : end_idx],
            "gt_rgb": self.rgb_frames_img[start_idx : end_idx],
            "gt_albedo": self.albedo_frames_img[start_idx : end_idx],
            "height": self.frame_h,
            "width": self.frame_w,
            "elevation": self.elevation_deg[start_idx : end_idx],
            "azimuth": self.azimuth_deg[start_idx : end_idx],
            "camera_distances": self.camera_distances[start_idx : end_idx],
        }
...
class MultiviewIterableDataset(IterableDataset):
    def __init__(self, cfg: Any) -> None:
        ...
        # self.multiview_RD_cfg = parse_structured(MultiviewsDataModuleConfig, self.cfg.multiview_RD)
        # self.multiview_RD_cfg.dataroot = os.path.join(self.multiview_RD_cfg.dataroot, "RD")
        # self.multiview_RD_IterableDataset = RD_MultiviewIterableDataset(self.multiview_RD_cfg)
    ...
    def collate(self, batch):
        ...
        # multiview_RD = self.multiview_RD_IterableDataset.collate(batch)
        return {"uncond_HN": uncond_HN}
...
@register("multiview-camera-cloth-datamodule")
