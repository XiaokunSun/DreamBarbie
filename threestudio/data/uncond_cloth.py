...
from threestudio.data.multiview_RD import Dream3DOrthogonalIterableDataset
from threestudio.data.multiview_RD import RandomCameraDataModuleConfig as Dream3DOrthogonalConfig
from threestudio.data.uncond_RD import RandomCameraMultiDataset
import threestudio
...
class RandomCameraDataModuleConfig:
    ...
    fullbody_part_ratio: dict = field(default_factory=dict)
    test_part_ratio: dict = field(default_factory=dict)
    multiview_RD_flag: bool = False
    multiview_RD: dict = field(default_factory=dict)
    sim_flag: bool = False
    RD_prob: float = 0.5
...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    def __init__(self, cfg: Any) -> None:
        ...
        self.part_ratio = self.cfg.fullbody_part_ratio.ratio[0]

        if self.cfg.multiview_RD_flag:
            self.multiview_RD_cfg = parse_structured(Dream3DOrthogonalConfig, self.cfg.multiview_RD)
            self.multiview_RD_IterableDataset = Dream3DOrthogonalIterableDataset(self.multiview_RD_cfg)
        else:
            self.multiview_RD_IterableDataset = None
    ...
    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False):
        ...
        for idx, step in enumerate(self.cfg.fullbody_part_ratio.step):
            if global_step < step:
                break
            ratio = self.cfg.fullbody_part_ratio.ratio[idx]
        self.part_ratio = ratio

        if self.multiview_RD_IterableDataset is not None:
            self.multiview_RD_IterableDataset.update_step(epoch, global_step, on_load_weights)
    ...
    def collate(self, batch) -> Dict[str, Any]:
        if self.cfg.sim_flag:
            RD_flag = True
            HN_flag = True
        else:
            RD_flag = random.random() < self.cfg.RD_prob
            HN_flag = not RD_flag
        out = {}
        if HN_flag:
            ...
            if self.cfg.sampling_type == "head_only":
                ...
                human_part = np.random.choice([0, 1, 2, 3], p=self.part_ratio)
            ...
            out.update({"uncond_HN": HN_output})

        if RD_flag and self.multiview_RD_IterableDataset is not None:
            multiview_RD_output = self.multiview_RD_IterableDataset.collate(batch)
            out.update({"multiview_RD": multiview_RD_output})  
...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        self.part_ratio = self.cfg.test_part_ratio
        if split == "val":
        ...
        if self.cfg.random_test:
            if self.cfg.sampling_type == "head_only":
                ...
                n_views_part = torch.tensor([int(self.n_views * ratio) for ratio in self.part_ratio], dtype=torch.int)
                assert n_views_part.sum().item() == self.n_views
        ...
        if self.split == "val":
            ...
            if self.cfg.random_test and self.cfg.sampling_type == "full_body":
        ...
        if self.split == "test" and self.cfg.random_test and self.cfg.sampling_type == "full_body":
            elevation_deg: Float[Tensor, "B"] = (
        ...
        if self.split == "test" and self.cfg.random_test and self.cfg.sampling_type == "full_body":
            if self.cfg.sampling_type == "head_only":
                ...
                focal_scale_p1 = torch.full([n_views_part[0].item()], 1.0)
                focal_scale_p2 = torch.full([n_views_part[1].item()], 4.0)
                focal_scale_p3 = torch.full([n_views_part[2].item()], 2.0)
                focal_scale_p4 = torch.full([n_views_part[3].item()], 2.0)
                ...
                center[n_views_part[0].item():n_views_part[0].item()+n_views_part[1].item(), 2] += 0.6
                center[n_views_part[0].item()+n_views_part[1].item():n_views_part[0].item()+n_views_part[1].item()+n_views_part[2].item(), 2] += 0.3
                center[n_views_part[0].item()+n_views_part[1].item()+n_views_part[2].item():, 2] -= 0.5
        ...
        # if self.cfg.multiview_RD_flag and self.split == "test":
        #     self.multiview_RD_cfg = parse_structured(Dream3DOrthogonalConfig, self.cfg.multiview_RD)
        #     if self.cfg.random_test:
        #         self.multiview_RD_Dataset = RandomCameraMultiDataset(self.multiview_RD_cfg, "multiview_test")
        #     else:
        #         self.multiview_RD_Dataset = RandomCameraMultiDataset(self.multiview_RD_cfg, "test")
        # elif self.cfg.multiview_RD_flag and self.split == "val":
        #     self.multiview_RD_cfg = parse_structured(Dream3DOrthogonalConfig, self.cfg.multiview_RD)
        #     self.multiview_RD_Dataset = RandomCameraMultiDataset(self.multiview_RD_cfg, "test")
        # else:
        #     self.multiview_RD_Dataset = None
    ...
    def __getitem__(self, index):
        ...
        out.update({"uncond_HN": uncond_HN_out})
        # if self.multiview_RD_Dataset is not None:
        #     multiview_RD_output = self.multiview_RD_Dataset[index]
        #     out.update({"multiview_RD": multiview_RD_output})  
        return out

    def collate(self, batch):
        batch = torch.utils.data.default_collate(batch)
        batch["uncond_HN"].update({"height": self.cfg.eval_height, "width": self.cfg.eval_width})
        # if self.multiview_RD_Dataset is not None:
        #     batch["multiview_RD"].update({"height": self.multiview_RD_cfg.eval_height, "width": self.multiview_RD_cfg.eval_width})
        return batch


@register("random-camera-cloth-datamodule")
