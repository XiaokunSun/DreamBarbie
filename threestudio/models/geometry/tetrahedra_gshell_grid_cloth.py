...
from threestudio.models.geometry.implicit_sdf_cloth import ImplicitSDF
from threestudio.models.geometry.implicit_volume import ImplicitVolume
from threestudio.models.isosurface import MarchingTetrahedraHelper, MarchingTetrahedraShellHelper, GshellHelper
...
class TetrahedraSDFGrid(BaseExplicitGeometry):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.isosurface_resolution.split(":")[0] == "cube":
            ...
            self.isosurface_helper = GshellHelper(resolution=tmp_res, tets_path=f"load/tets/{tmp_res}_tets.npz").to(self.device)
            ...
            self.isosurface_helper = GshellHelper(
                resolution=None, tets_path=None, mesh=None, tet_shell_offset=float(tmp_para[0]), tet_shell_decimate=float(tmp_para[1]), tet_grid_volume=float(tmp_para[2]), save_dir_path=self.cfg.test_save_path.replace("cache", "obj/tetgen").replace("stage2-coarse-texture", "stage1-geometry")
            ).to(self.device)
        self.sdf: Float[Tensor, "Nv 1"]
        self.msdf: Float[Tensor, "Nv 1"]
        ...
        self.nonwt_mesh: Optional[Mesh] = None
        self.wt_mesh: Optional[Mesh] = None
    ...
    def isosurface(self, mode="nonwt") -> Mesh:
        ...
        if self.cfg.fix_geometry and self.nonwt_mesh is not None and self.wt_mesh is not None:
            if mode == "wt":
                return self.wt_mesh
            else:
                return self.nonwt_mesh, {"mesh_watertight": self.wt_mesh}
        else:
            mesh, extra = self.isosurface_helper.forward_gshell(self.sdf, self.msdf, self.deformation, True)
            mesh.v_pos = scale_tensor(
                mesh.v_pos, self.isosurface_helper.points_range, self.isosurface_bbox
            )
            extra["mesh_watertight"].v_pos = scale_tensor(
                extra["mesh_watertight"].v_pos, self.isosurface_helper.points_range, self.isosurface_bbox
            )
            self.nonwt_mesh = mesh
            self.wt_mesh = extra["mesh_watertight"]
            if mode == "wt":
                return self.wt_mesh
            else:
                return self.nonwt_mesh, {"mesh_watertight": self.wt_mesh}
    ...
    def create_from(
        ...
        **kwargs,
        ...
        if other.__class__.__name__ == "TetrahedraSDFGrid":
            ...
            instance.msdf.data = other.msdf.data.clone()
            if (
            ...
            return instance
        elif other.__class__.__name__ == "ImplicitSDF":
            instance = TetrahedraSDFGrid(cfg, **kwargs)
            assert other.cfg.isosurface_resolution == instance.cfg.isosurface_resolution
            if other.cfg.isosurface_method != "gshell":
                other.cfg.isosurface_method = "gshell"
                threestudio.warn(
                    f"Override isosurface_method of the source geometry to 'gshell'"
                )
            ...
            mesh, extra = other.isosurface("nonwt+wt")

            instance.isosurface_bbox = mesh.extras["bbox"]
            instance.sdf.data = mesh.extras["grid_sdf"].to(instance.sdf.data)
            instance.msdf.data = mesh.extras["grid_msdf"].to(instance.msdf.data)
            if (
                instance.cfg.isosurface_deformable_grid
                and other.cfg.isosurface_deformable_grid
            ):
