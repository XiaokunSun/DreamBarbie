...
from threestudio.models.isosurface import MarchingTetrahedraHelper, MarchingTetrahedraShellHelper
...
@threestudio.register("tetrahedra-sdf-grid-cloth")
    ...
    def configure(self) -> None:
        ...
        if self.cfg.isosurface_resolution.split(":")[0] == "cube":
            ...
            self.isosurface_helper = MarchingTetrahedraShellHelper(
                None, float(tmp_para[0]), float(tmp_para[1]), float(tmp_para[2]), self.cfg.test_save_path.replace("cache", "obj/tetgen").replace("stage2-coarse-texture", "stage1-geometry")
            ).to(self.device)
