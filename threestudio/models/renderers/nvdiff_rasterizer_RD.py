...
@threestudio.register("nvdiff-rasterizer-RD")
    ...
    def forward(
        ...
        **kwargs
        ...
        if hasattr(self.geometry, 'msdf') or hasattr(self.geometry, 'msdf_network'):
            if gshell_mode == "wt":
                mesh = self.geometry.isosurface(gshell_mode)
            else:
                mesh, extra = self.geometry.isosurface(gshell_mode)
        else:
