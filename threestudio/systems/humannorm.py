...
class HumanNorm(BaseLift3DSystem):
    ...
    def on_fit_start(self) -> None:
        ...
        if not self.cfg.texture:
            ...
            if self.geometry.opt_smplx_flag:
                os.makedirs(os.path.join(self.obj_dir_path, "opt_smplx"), exist_ok=True)
            else:
    ...
    def on_before_optimizer_step(self, optimizer):
        with torch.no_grad():
            if not self.cfg.texture:
                if self.geometry.opt_smplx_flag and self.true_global_step <= self.geometry.cfg.opt_smplx_step_num:
    ...
    def training_step(self, batch, batch_idx):
        if not self.cfg.texture:
            if self.geometry.opt_smplx_flag and self.true_global_step <= self.geometry.cfg.opt_smplx_step_num:   
                loss = 0.0
                mesh = self.geometry.get_opt_smplx_mesh(offsets_flag = False, filter_flag = False)
                ...
                if self.true_global_step % 100 == 0:
                    ...
                    tri_mesh.export(os.path.join(self.obj_dir_path, "opt_smplx", f"smplx_{self.true_global_step}.obj"))

                if self.true_global_step == self.geometry.cfg.opt_smplx_step_num:
                    tri_mesh = trimesh.Trimesh(
                            vertices=mesh.v_pos.detach().cpu().numpy(),
                            faces=mesh.t_pos_idx.detach().cpu().numpy(),
                        )
                    self.geometry.initialize_shape_from_smplx(tri_mesh, self.true_global_step)
                    if self.geometry.cfg.isosurface_resolution.split(":")[0] == "shell":
                        ...
                        self.geometry._initilize_isosurface_helper(tri_mesh, os.path.join(self.obj_dir_path, "tetgen"))
        ...
        if not self.cfg.texture:  # geometry training
            ...
            if self.C(self.cfg.loss.lambda_smplx_local) > 0 and self.true_global_step >= self.cfg.geometry.start_smplx_loss_step:
                loss_smplx_local = self.geometry.get_smplx_local_loss()
                self.log("train/loss_smplx_local", loss_smplx_local)
                loss += loss_smplx_local * self.C(
                    self.cfg.loss.lambda_smplx_local
                )

            if self.C(self.cfg.loss.lambda_smplx_global) > 0 and self.true_global_step >= self.cfg.geometry.start_smplx_loss_step:
                loss_smplx_global = self.geometry.get_smplx_global_loss()
                self.log("train/loss_smplx_global", loss_smplx_global)
                loss += loss_smplx_global * self.C(
                    self.cfg.loss.lambda_smplx_global
                )
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.cfg.exporter.save_flag and batch['index'][0] == 0:
            if not self.cfg.texture:
                ...
                mesh.export(os.path.join(obj_dir_path, f"human.obj"))
            else:
                obj_dir_path = os.path.join(self.get_save_dir(), "obj", "result")
                os.makedirs(obj_dir_path, exist_ok=True)
                ...
                self.save_obj(os.path.join("obj", "result", "human.obj"), **output[0].params)
