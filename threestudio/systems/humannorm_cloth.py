...
@threestudio.register("humannorm-cloth-system")
class HumanNorm_cloth(BaseLift3DSystem):
    @dataclass
    ...
    def forward(self, batch: Dict[str, Any], render_mode="cloth_c+comp+RD", render_asset_idx=None) -> Dict[str, Any]:
        out = {}

        if "comp" in render_mode and "uncond_HN" in batch.keys():
            HN_render_out = self.renderer(**batch["uncond_HN"], render_rgb=self.cfg.texture, render_mode=render_mode, render_asset_idx=render_asset_idx)
            out.update({"HN_render_out": HN_render_out})
        
        if "RD" in render_mode and "multiview_RD" in batch.keys():
            RD_render_out = self.renderer_RD(**batch["multiview_RD"], render_rgb=self.cfg.texture)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.C(self.cfg.loss.lambda_l1) > 0 and self.C(self.cfg.loss.lambda_p) > 0:
            ...
            out = self(batch, render_mode="cloth_c+comp+RD")
    ...
    def validation_step(self, batch, batch_idx):
        if self.C(self.cfg.loss.lambda_l1) > 0 and self.C(self.cfg.loss.lambda_p) > 0:
            render_out = self.renderer.render_human_and_cloth(**batch["uncond_HN"], render_rgb=self.cfg.texture)
            out = self(batch, render_mode="comp")
            HN_render_out = out["HN_render_out"]
            out1 = self(batch, render_mode="comp", render_asset_idx=list(range(1, self.asset_num)))
            ...
            out = self(batch, render_mode="asset+cloth_c+comp")
            HN_render_out = out["HN_render_out"]
            if self.cfg.texture:
                self.save_image_grid(
                    f"it{self.true_global_step}-val-color/{batch['uncond_HN']['index'][0]}.png",
