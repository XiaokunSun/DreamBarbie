...
@threestudio.register("humannorm-cloth-gshell-system")
class HumanNorm_cloth_gshell(BaseLift3DSystem):
    @dataclass
    class Config(BaseLift3DSystem.Config):
        ...
        cloth: dict = field(default_factory=dict)
        RD: dict = field(default_factory=dict)
        remove_img: bool = True
        gshell_mode: str = "nonwt"
        temp_loss_mode: str = "in"
        msdf_temp_loss_mode: str = "in"
    ...
    def configure(self):
        ...
        for i in range(self.asset_num):
            ...
            if self.cfg.asset.name[i] == "human":
            ...
            if hasattr(asset_geometry,'_initilize_isosurface_helper'):
                asset_geometry._initilize_isosurface_helper(mesh = None, save_dir_path = asset_geometry.cfg.test_save_path.replace("cache", "obj/tetgen"))
            asset_geometry = asset_geometry.to(get_device())
            if self.C(self.cfg.loss.lambda_l1) > 0 and self.C(self.cfg.loss.lambda_p) > 0 and self.cfg.asset.name[i] != "human":
                asset_geometry.requires_grad_(True)
            else:
        ...
        if (
            ...
            asset_geometry.do_update_step(epoch, global_step, on_load_weights=True)
            if hasattr(prev_geometry, '_initilize_isosurface_helper'):
                prev_geometry._initilize_isosurface_helper(mesh = None, save_dir_path = self.cfg.geometry_convert_from.replace("ckpts/last.ckpt", "save/obj/tetgen"))
    ...
    def forward(self, batch: Dict[str, Any], render_mode="cloth_c+comp+RD", render_asset_idx=None, gshell_mode="nonwt") -> Dict[str, Any]:
        out = {}

        if "comp" in render_mode and "uncond_HN" in batch.keys():
            HN_render_out = self.renderer(**batch["uncond_HN"], render_rgb=self.cfg.texture, render_mode=render_mode, render_asset_idx=render_asset_idx, gshell_mode=gshell_mode)
            out.update({"HN_render_out": HN_render_out})
        
        if "RD" in render_mode and "multiview_RD" in batch.keys():
            RD_render_out = self.renderer_RD(**batch["multiview_RD"], render_rgb=self.cfg.texture, gshell_mode=gshell_mode)
            out.update({"RD_render_out": RD_render_out})
    ...
    def on_fit_start(self) -> None:
        ...
        if self.cfg.guidance_RD_StableDiffusion_type != "":
            self.prompt_processor_RD_StableDiffusion = threestudio.find(self.cfg.prompt_processor_RD_StableDiffusion_type)(
                self.cfg.prompt_processor_RD_StableDiffusion
            )
            self.guidance_RD_StableDiffusion = threestudio.find(self.cfg.guidance_RD_StableDiffusion_type)(self.cfg.guidance_RD_StableDiffusion)
        else:
            self.prompt_processor_RD_StableDiffusion = None
            self.guidance_RD_StableDiffusion = None
            
        if self.cfg.guidance_RD_NormalDepth_type != "":
            self.prompt_processor_RD_NormalDepth = threestudio.find(self.cfg.prompt_processor_RD_NormalDepth_type)(
                self.cfg.prompt_processor_RD_NormalDepth
            )
            self.guidance_RD_NormalDepth = threestudio.find(self.cfg.guidance_RD_NormalDepth_type)(self.cfg.guidance_RD_NormalDepth)
        else:
            self.prompt_processor_RD_NormalDepth = None
            self.guidance_RD_NormalDepth = None

        if self.cfg.guidance_RD_Albedo_type != "":
            self.prompt_processor_RD_Albedo = threestudio.find(self.cfg.prompt_processor_RD_Albedo_type)(
                self.cfg.prompt_processor_RD_Albedo
            )
            self.guidance_RD_Albedo = threestudio.find(self.cfg.guidance_RD_Albedo_type)(self.cfg.guidance_RD_Albedo)
        else:
            self.prompt_processor_RD_Albedo = None
            self.guidance_RD_Albedo = None
        ...
        if not self.cfg.texture:
            ...
            # self.cloth_mesh_trimesh_list = []
            # self.cloth_mesh_bbox_list = []
            # self.cloth_mesh_sdf_list = []
            # for i in range(len(self.asset_system_cfg_list)):
            #     if i == 0:
            #         self.human_mesh = self.asset_geometry_list[i].isosurface()
            #         self.human_mesh_trimesh = trimesh.Trimesh(vertices=self.human_mesh.v_pos.cpu().numpy(), faces=self.human_mesh.t_pos_idx.cpu().numpy())
            #         self.human_mesh_sdf = SDF(self.human_mesh_trimesh.vertices, self.human_mesh_trimesh.faces)
            #     else:

            #         cloth_mesh = self.asset_geometry_list[i].isosurface()
            #         tmp_vertices = cloth_mesh.v_pos.detach() * self.asset_geometry_list[i].scale + self.asset_geometry_list[i].trans
            #         self.cloth_mesh_trimesh_list.append(trimesh.Trimesh(vertices=tmp_vertices.cpu().numpy(), faces=cloth_mesh.t_pos_idx.cpu().numpy()))
            #         self.cloth_mesh_bbox_list.append([tmp_vertices.min(0)[0], tmp_vertices.max(0)[0]])
            #         self.cloth_mesh_sdf_list.append(SDF(self.cloth_mesh_trimesh_list[-1].vertices, self.cloth_mesh_trimesh_list[-1].faces))
            self.human_mesh = self.asset_geometry_list[0].isosurface()
            self.human_sdf = SDF(self.human_mesh.v_pos.cpu().numpy(), self.human_mesh.t_pos_idx.cpu().numpy())
            self.human_sdf_network = self.asset_geometry_list[0]
            if self.cfg.asset.name[0] == "human" and self.cfg.geometry.shape_init.startswith("human:"):
                if self.cfg.geometry.shape_init.startswith("human:opt_smplx"):
                    opt_smplx_mesh = self.asset_geometry_list[0].get_opt_smplx_mesh(offsets_flag=True, filter_flag=True)
                    self.geometry.initialize_shape(verts=self.human_mesh.v_pos.detach().cpu().numpy(), faces=self.human_mesh.t_pos_idx.detach().cpu().numpy(), system_cfg=self.cfg, opt_smplx_mesh=opt_smplx_mesh)
                    os.makedirs(os.path.join(self.obj_dir_path, "result"), exist_ok=True)
                    trimesh.Trimesh(vertices=opt_smplx_mesh.v_pos.detach().cpu().numpy(), faces=opt_smplx_mesh.t_pos_idx.detach().cpu().numpy()).export(os.path.join(self.obj_dir_path, "result", "smplx.obj"))
                else:
                    self.geometry.initialize_shape(verts=self.human_mesh.v_pos.detach().cpu().numpy(), faces=self.human_mesh.t_pos_idx.detach().cpu().numpy(), system_cfg=self.cfg)
            else:
                self.geometry.initialize_shape(system_cfg=self.cfg)

            if self.cfg.asset.name[0] == "human" and self.cfg.geometry.shape_init.startswith("human:"):
                if self.cfg.geometry_type == "implicit-gshell-cloth":
                    if self.geometry.cfg.isosurface_resolution.split(":")[0] == "shell":
                        os.makedirs(os.path.join(self.obj_dir_path, "tetgen"), exist_ok=True)
                        self.geometry.isosurface_helper = None
                        tmp_res = self.geometry.cfg.isosurface_resolution
                        self.geometry.cfg.isosurface_resolution = "cube:256"
                        tmp_mesh = self.geometry.isosurface(mode="wt")
                        mesh = trimesh.Trimesh(
                                vertices=tmp_mesh.v_pos.detach().cpu().numpy(),
                                faces=tmp_mesh.t_pos_idx.detach().cpu().numpy(),
                            )
                        mesh.export(f"{self.geometry.cfg.test_save_path}/obj/init_mesh/init_close_mesh_scale_cube256.obj")
                        self.geometry.cfg.isosurface_resolution = tmp_res
                        self.geometry.isosurface_helper = None
                        self.geometry._initilize_isosurface_helper(trimesh.load(f"{self.geometry.cfg.test_save_path}/obj/init_mesh/init_close_mesh_scale_cube256.obj"), os.path.join(self.obj_dir_path, "tetgen"))
                        if self.geometry.cfg.msdf_type == "explicit":
                            self.geometry._initilize_explicit_msdf(mode=self.geometry.cfg.init_msdf_type)
                        elif self.geometry.cfg.msdf_type == "implicit":
                            self.geometry._initilize_implicit_msdf(mode=self.geometry.cfg.init_msdf_type)
                    else:
                        self.geometry._initilize_isosurface_helper()
                        if self.geometry.cfg.msdf_type == "explicit":
                            self.geometry._initilize_explicit_msdf(mode=self.geometry.cfg.init_msdf_type)
                        elif self.geometry.cfg.msdf_type == "implicit":
                            self.geometry._initilize_implicit_msdf(mode=self.geometry.cfg.init_msdf_type)
                    cloth_c_mesh_nonwt, extra = self.geometry.isosurface(mode="nonwt+wt")
                    os.makedirs(os.path.join(self.obj_dir_path, "init_cloth_c"), exist_ok=True)
                    nonwt_mesh = trimesh.Trimesh(
                            vertices=cloth_c_mesh_nonwt.v_pos.detach().cpu().numpy(),
                            faces=cloth_c_mesh_nonwt.t_pos_idx.detach().cpu().numpy(),
                        )
                    nonwt_mesh.export(os.path.join(self.obj_dir_path, "init_cloth_c", f"nonwt_mesh.obj"))
                    wt_mesh = trimesh.Trimesh(
                            vertices=extra["mesh_watertight"].v_pos.detach().cpu().numpy(),
                            faces=extra["mesh_watertight"].t_pos_idx.detach().cpu().numpy(),
                        )
                    wt_mesh.export(os.path.join(self.obj_dir_path, "init_cloth_c", f"wt_mesh.obj"))
                else:
                    if self.geometry.cfg.isosurface_resolution.split(":")[0] == "shell":
                        os.makedirs(os.path.join(self.obj_dir_path, "tetgen"), exist_ok=True)
                        self.geometry.isosurface_helper = None
                        tmp_res = self.geometry.cfg.isosurface_resolution
                        self.geometry.cfg.isosurface_resolution = "cube:256"
                        tmp_mesh = self.geometry.isosurface()
                        mesh = trimesh.Trimesh(
                                vertices=tmp_mesh.v_pos.detach().cpu().numpy(),
                                faces=tmp_mesh.t_pos_idx.detach().cpu().numpy(),
                            )
                    ...
                    mesh = trimesh.Trimesh(
                            vertices=cloth_c_mesh.v_pos.detach().cpu().numpy(),
                            faces=cloth_c_mesh.t_pos_idx.detach().cpu().numpy(),
                        )
                    os.makedirs(os.path.join(self.obj_dir_path, "init_cloth_c"), exist_ok=True)
                    mesh.export(os.path.join(self.obj_dir_path, "init_cloth_c", f"init_cloth_c.obj"))
    ...
    #             if self.geometry.cfg.msdf_type == "explicit":
    #                 print(torch.abs(self.geometry.msdf.grad).mean().item())
    #                 print(torch.abs(self.geometry.msdf).mean().item())
    #             elif self.geometry.cfg.msdf_type == "implicit":
    #                 print(torch.abs(self.geometry.msdf_network.layers[0].weight.grad).mean().item())
    #                 print(torch.abs(self.geometry.msdf_network.layers[0].weight).mean().item())
    #         else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.C(self.cfg.loss.lambda_l1) > 0 and self.C(self.cfg.loss.lambda_p) > 0:
            out = self(batch, render_mode="comp", gshell_mode=self.cfg.gshell_mode)
        else:
            out = self(batch, render_mode="cloth_c+comp+RD", gshell_mode=self.cfg.gshell_mode)
        ...
        if "RD_render_out" in out.keys():
            RD_out = out["RD_render_out"]
            if not self.cfg.texture: 
                if self.C(self.cfg.loss.lambda_sds_RD_StableDiffusion) > 0:
                    prompt_utils = self.prompt_processor_RD_StableDiffusion()
                    if self.true_global_step < self.cfg.RD.latent_steps:
                        normal = RD_out["sd_comp_normal"] * 2.0 - 1.0
                        guidance_inp = torch.cat([normal, RD_out["opacity"]], dim=-1)
                        guidance_out = self.guidance_RD_StableDiffusion(
                        ...
                        loss_dict.update({"loss_sds_RD_StableDiffusion":guidance_out["loss_sds"]})
                    else:
                        guidance_inp = RD_out["sd_comp_normal"]
                        guidance_out = self.guidance_RD_StableDiffusion(
                            {"comp_rgb": guidance_inp},
                            prompt_utils,
                            **batch["multiview_RD"],
                        ...
                        loss_dict.update({"loss_sds_RD_StableDiffusion":guidance_out["loss_sds"]})
                if self.C(self.cfg.loss.lambda_sds_RD_NormalDepth) > 0:
                    prompt_utils = self.prompt_processor_RD_NormalDepth()
                    normal = RD_out["comp_normal"] * 2.0 - 1.0
                    disparity = RD_out["mv_disparity"] * 2.0 - 1.0
                    guidance_inp = torch.cat([normal, disparity], dim=-1)
                    guidance_out = self.guidance_RD_NormalDepth(
                        {"comp_rgb": guidance_inp},
                        prompt_utils,
                        **batch["multiview_RD"],
                        rgb_as_latents=True,
                    ...
                    loss_dict.update({"loss_sds_RD_NormalDepth":guidance_out["loss_sds"]})
                if mesh is None:
                    mesh = RD_out["mesh"]
            else:
                if self.C(self.cfg.loss.lambda_sds_RD_StableDiffusion) > 0:
                    prompt_utils = self.prompt_processor_RD_StableDiffusion()
                    guidance_inp = RD_out["comp_rgb"]
                    guidance_out = self.guidance_RD_StableDiffusion(
                        {"comp_rgb": guidance_inp},
                        prompt_utils,
                        **batch["multiview_RD"],
                    ...
                    loss_dict.update({"loss_sds_RD_StableDiffusion":guidance_out["loss_sds"]})
                if self.C(self.cfg.loss.lambda_sds_RD_Albedo) > 0:
                    prompt_utils = self.prompt_processor_RD_Albedo()
                    guidance_out = self.guidance_RD_Albedo(
                        RD_out,
                        prompt_utils,
                        **batch["multiview_RD"],
                        rgb_as_latents=False,
                    ...
                    loss_dict.update({"loss_sds_RD_Albedo":guidance_out["loss_sds"]})
                if self.C(self.cfg.loss.lambda_l1_RD_StableDiffusion) > 0 and self.C(self.cfg.loss.lambda_p_RD_StableDiffusion) > 0:
                    prompt_utils = self.prompt_processor_RD_StableDiffusion()
                    guidance_inp = RD_out["comp_rgb"]
                    guidance_out = self.guidance_RD_StableDiffusion(
                        {"comp_rgb": guidance_inp},
                    ...
                    loss_dict.update({"loss_l1_RD_StableDiffusion":guidance_out["loss_l1"]})
                    loss_dict.update({"loss_p_RD_StableDiffusion":guidance_out["loss_p"]})
                if self.C(self.cfg.loss.lambda_l1_RD_Albedo) > 0 and self.C(self.cfg.loss.lambda_p_RD_Albedo) > 0:
                    prompt_utils = self.prompt_processor_RD_Albedo()
                    guidance_out = self.guidance_RD_Albedo(
                        RD_out,
                        prompt_utils,
                    ...
                    loss_dict.update({"loss_l1_RD_Albedo":guidance_out["loss_l1"]})
                    loss_dict.update({"loss_p_RD_Albedo":guidance_out["loss_p"]})
        ...
        if not self.cfg.texture:  # geometry training
            ...
            if self.C(self.cfg.loss.lambda_temp) > 0 and self.true_global_step >= self.cfg.geometry.start_temp_loss_step:
                loss_temp = self.geometry.get_temp_loss(loss_mode=self.cfg.temp_loss_mode)
                loss_dict.update({"loss_temp": loss_temp})

            if self.C(self.cfg.loss.lambda_msdf_temp) > 0:
                loss_msdf_temp = self.geometry.get_msdf_temp_loss(loss_mode=self.cfg.msdf_temp_loss_mode)
                loss_dict.update({"loss_msdf_temp": loss_msdf_temp})

            if self.C(self.cfg.loss.lambda_collision) > 0:   
                cloth_a_mesh_v = HN_out["cloth_c_mesh"].v_pos * self.geometry.scale + self.geometry.trans
                GT_sdf = self.human_sdf_network.forward_sdf(cloth_a_mesh_v)
                loss_collision = F.relu( - GT_sdf).sum()
            ...
            #     sample_pts = (((torch.rand((200000, 3), dtype=torch.float32).to(self.device) - 0.5) * scale + trans) * )
            #     in_flag = torch.zeros(sample_pts.shape[0], dtype=torch.bool).to(self.device)
            #     for i in range(len(self.asset_geometry_list)):
            #         if i == 0:
            #             sdf_gt = torch.from_numpy(-self.human_mesh_sdf(sample_pts.cpu().numpy())).to(sample_pts)[..., None]
            #             in_flag = torch.logical_or(in_flag, (sdf_gt < 0).squeeze())
            #         else:
            #             sample_pts_min, sample_pts_max = sample_pts.min(0)[0], sample_pts.max(0)[0]
            #             if (sample_pts_max >= self.cloth_mesh_bbox_list[i-1][0]).all() and (sample_pts_min <= self.cloth_mesh_bbox_list[i-1][1]).all():
            #                 sdf_gt = torch.from_numpy(-self.cloth_mesh_sdf_list[i-1](sample_pts.cpu().numpy())).to(sample_pts)[..., None]
            #                 in_flag = torch.logical_or(in_flag, (sdf_gt < 0).squeeze())
            #     sample_pts = sample_pts[in_flag, :]
            #     cloth_sdf = self.geometry.forward_sdf(sample_pts)
            #     epsilon = torch.ones_like(cloth_sdf).to(self.device) * 0
            #     zero_tensor = torch.zeros_like(cloth_sdf).to(self.device)
            #     loss_collision = torch.max(zero_tensor, epsilon - cloth_sdf).sum()
            #     loss_dict.update({"loss_collision": loss_collision})
    ...
    def validation_step(self, batch, batch_idx):
        if self.C(self.cfg.loss.lambda_l1) > 0 and self.C(self.cfg.loss.lambda_p) > 0:
            render_out = self.renderer.render_human_and_cloth(**batch["uncond_HN"], render_rgb=self.cfg.texture, gshell_mode="nonwt")
            out = self(batch, render_mode="comp", gshell_mode="nonwt")
            HN_render_out = out["HN_render_out"]
            out1 = self(batch, render_mode="comp", render_asset_idx=list(range(1, self.asset_num)), gshell_mode="nonwt")
            HN_render_out1 = out1["HN_render_out"]
            self.save_image_grid(
                ...
                (
                    [
                        {
                            "type": "rgb",
                            "img": HN_render_out1["comp_render_rgb"][0],
                            "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                        }
                    ] if "comp_render_rgb" in HN_render_out1 else []
                )
                +
                (
                    [
                        {
                            "type": "rgb",
                            "img": asset_render_out["rgb"][0],
                            "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                        } 
                        for asset_render_out in render_out ]
                ),
                name="validation_step",
                step=self.true_global_step,
            )
        else:
            out = self(batch, render_mode="asset+cloth_c+comp", gshell_mode="nonwt")
            HN_render_out = out["HN_render_out"]
            out1 = self(batch, render_mode="asset+cloth_c+comp", gshell_mode="wt")
            HN_render_out1 = out1["HN_render_out"]
            if self.cfg.texture:
                self.save_image_grid(
                    f"it{self.true_global_step}-val-color/{batch['uncond_HN']['index'][0]}.png",
                    (
                        [
                            {
                                "type": "rgb",
                                "img": HN_render_out1["cloth_c_render_rgb"][0],
                                "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                            }
                        ] if "cloth_c_render_rgb" in HN_render_out1 else []
                    )
                    + (
                        [
                            {
                                "type": "rgb",
                                "img": HN_render_out["cloth_c_render_rgb"][0],
                                "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                            }
                        ] if "cloth_c_render_rgb" in HN_render_out else []
                    )
                    + (
                        [
                            {
                                "type": "rgb",
                                "img": HN_render_out["cloth_a_render_rgb"][0],
                                "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                            }
                        ] if "cloth_a_render_rgb" in HN_render_out else []
                    )
                    + (
                        [
                            {
                ...
                self.save_image_grid(
                    ...
                    ([
                        {
                            "type": "rgb",
                            "img": HN_render_out1["cloth_c_render_normal"][0] + (1 - HN_render_out1["cloth_c_render_mask"][0, :, :, :]),
                            "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                        }
                    ] if "cloth_c_render_normal" in HN_render_out1 else [])
                    + ([
                        {
                            "type": "rgb",
                            "img": HN_render_out["cloth_c_render_normal"][0] + (1 - HN_render_out["cloth_c_render_mask"][0, :, :, :]),
                            "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                        },
                    ] if "cloth_c_render_normal" in HN_render_out else [])
                    + ([
                        {
                            "type": "rgb",
                            "img": HN_render_out["cloth_a_render_normal"][0] + (1 - HN_render_out["cloth_a_render_mask"][0, :, :, :]),
                            "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                        },
                    ] if "cloth_a_render_normal" in HN_render_out else [])
                    + ([
                        {
                            "type": "rgb",
    ...
    def test_step(self, batch, batch_idx):
        if 'focal' in batch['uncond_HN'] and self.cfg.texture:
            out = self(batch, render_mode="asset+cloth_c+comp", gshell_mode="nonwt+wt")
        else:
            out = self(batch, render_mode="asset+cloth_c+comp", gshell_mode="nonwt+wt")
        HN_render_out = out["HN_render_out"]
        # RD_render_out = out["RD_render_out"]
        if self.cfg.exporter.save_flag and batch['uncond_HN']['index'][0] == 0:
            ...
            for idx, mesh in enumerate(HN_render_out["comp_mesh_list"]):
                os.makedirs(os.path.join(self.obj_dir_path, "result"), exist_ok=True)
                if self.cfg.texture:
                    if self.cfg.exporter.fmt == "obj":
                        self.cfg.exporter.save_normal = False
                        self.cfg.exporter.save_uv = False
                        self.cfg.exporter.save_texture = True
                    elif self.cfg.exporter.fmt == "obj-mtl":
                        self.cfg.exporter.save_normal = True
                        self.cfg.exporter.save_uv = True
                        self.cfg.exporter.save_texture = True
                    if idx == 0:
                        ...
                        self.save_obj(os.path.join("obj", "result", f"comp_{idx}.obj"), **output[0].params, scale=scale.detach().cpu().numpy(), trans=trans.detach().cpu().numpy())
                        self.save_obj(os.path.join("obj", "result", f"cloth_{idx}.obj"), **output[0].params)
                else:
                    ...
                    #     trans = self.asset_geometry_list[idx].trans if idx < self.asset_num else self.geometry.trans
                    #     trimesh_mesh = trimesh.Trimesh(
                    #         vertices=(mesh.v_pos.detach().cpu().numpy() - trans.detach().cpu().numpy()) / scale.detach().cpu().numpy(),
                    #         faces=mesh.t_pos_idx.detach().cpu().numpy(),
                    #     )
                    #     trimesh_mesh.export(os.path.join(self.obj_dir_path, "result", f"cloth_{idx}.obj"))
                    pass

            if not self.cfg.texture:
                trimesh.Trimesh(vertices=HN_render_out["cloth_c_mesh"].v_pos.detach().cpu().numpy(), faces=HN_render_out["cloth_c_mesh"].t_pos_idx.detach().cpu().numpy()).export(os.path.join(self.obj_dir_path, "result", "nonwt_c.obj"))
                trimesh.Trimesh(vertices=(HN_render_out["cloth_c_mesh"].v_pos.detach().cpu().numpy() * self.geometry.scale.detach().cpu().numpy()) + self.geometry.trans.detach().cpu().numpy(), faces=HN_render_out["cloth_c_mesh"].t_pos_idx.detach().cpu().numpy()).export(os.path.join(self.obj_dir_path, "result", "nonwt_a.obj"))

                if HN_render_out["extra"] is not None:
                    trimesh.Trimesh(vertices=HN_render_out["extra"]["mesh_watertight"].v_pos.detach().cpu().numpy(), faces=HN_render_out["extra"]["mesh_watertight"].t_pos_idx.detach().cpu().numpy()).export(os.path.join(self.obj_dir_path, "result", "wt_c.obj"))
                    trimesh.Trimesh(vertices=(HN_render_out["extra"]["mesh_watertight"].v_pos.detach().cpu().numpy() * self.geometry.scale.detach().cpu().numpy()) + self.geometry.trans.detach().cpu().numpy(), faces=HN_render_out["extra"]["mesh_watertight"].t_pos_idx.detach().cpu().numpy()).export(os.path.join(self.obj_dir_path, "result", "wt_a.obj"))
            ...
            self.save_image_grid(
                ...
                (
                    [
                        {
                            "type": "rgb",
                            "img": HN_render_out1["comp_render_normal"][0] + (1 - HN_render_out1["comp_render_mask"][0, :, :, :]),
                            "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                        }
                    ] if "comp_render_normal" in HN_render_out1 else []
                ) 
                + 
                (
                    [
                        {
                            "type": "rgb",
                            "img": asset_render_out["normal"][0] + (1 - asset_render_out["opacity"][0, :, :, :]),
                            "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
                        } 
                        for asset_render_out in render_out ]
                ),
        ...
        if 'focal' in batch['uncond_HN'] and self.cfg.texture:
            if True:
                ...
                tmp_render_out = self.renderer(**tmp_batch, render_rgb=self.cfg.texture, render_mode="comp", gshell_mode="nonwt")
    ...
    def prepare_render_data(self):
        ...
        body_prop = 0.5
        cloth_prop = 1 - body_prop
        n_views_list = [int(body_prop * n_views)] + [int(cloth_prop * n_views / asset_scale.shape[0])] * asset_scale.shape[0]
